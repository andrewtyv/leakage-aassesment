...
ksla_from_inputs <- function(
  ...
  # 3) Read all traces into matrix N×S
  trace_list <- lapply(trace_files, scan, quiet=TRUE)
  traces     <- do.call(rbind, trace_list)
  
  # 4) Split into fixed vs random by first input
  ...
  # 5) Compute KS-statistic per sample
  S <- ncol(traces)
  ks_values <- numeric(S)
  for (t in seq_len(S)) {
